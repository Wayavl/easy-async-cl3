...
async fn test_pipes_producer_consumer() -> Result<(), ClError> {
    ...
    if device.get_opencl_version() < OpenCLVersion::V2_0 || !device.get_pipe_support().unwrap_or(false) {
        println!("Saltando test de Pipes: Hardware no compatible (se requiere OpenCL 2.0+ y soporte de Pipes).");
        return Ok(());
    }

    println!("Iniciando test de Pipes en {}", device.get_name().unwrap_or_default());
    ...
    let pipe = ClPipe::new(executor.get_context().as_ref(), &[MemoryFlags::ReadWrite], 4, 1024)?;
